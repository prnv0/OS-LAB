#include<stdio.h>
int process_size[10],block_size[10],block_allocated[10],process_alloc[10],frag[10]={-1};
void main()
{
int m,n,i,j;
...
for(j=0;j<m;j++){
	if(block_allocated[j]==0){
		if(((block_size[j]-process_size[i])>=0)&&(block_size[j]>block_size[large])){
			if(frag[j]!=-1&&frag[j]>=process_size[i])
				large=j;
		}
	}
}
